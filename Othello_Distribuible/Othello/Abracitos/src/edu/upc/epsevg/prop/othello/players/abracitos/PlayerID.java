...
/**
 * Clase IDS on s'implamenta el robot per guanyar al othello
 * @author Dario, Xavi
 */
public class PlayerID implements IPlayer, IAuto {
     ...
     * 1 - dimensio de la taula 150.001 (1 GB aprox), 2 threads
     * 2 - dimensio de la taula 500.009 (2 GB aprox), numero maxim de nuclis de la CPU
     */
    public PlayerID(int mode) {
        this.name = "Abracitos";
        if(mode == 1){
            this.dimensio_taula = 120011;
            this.num_threads = 2;
        }else if(mode == 2){
            this.dimensio_taula = 400009;
    ...
    public Move move(GameStatus gs) {
        ...
        //si som el jugador 1 jugarem de forma agresiva (tallant a maxims), si som el jugador 2 jugarem defensius (tallant a minims)
        this.profunditat_IDS = (jugador == CellType.PLAYER1 ? 2 : 1);
        
        ArrayList<Point> moves = gs.getMoves();
        this.fitxes_totals = moves.size();
        if(moves.isEmpty())
        {
    ...
    public Move novaTirada(AbracitosGame gs) {
        ...
        do{
            ...
            if(!timeout){
                taula_hash.actualitza(gs, new HashInfo(millor_heur, millor_posicio_prof, profunditat_IDS, gs.getBoard_color(), gs.getBoard_occupied(), 0, fitxes_totals));
    ...
    public int maximitza (AbracitosGame ag, int profunditat, int max_profunditat, int alpha, int beta){
        ...
        //si s'ha acabat el temps retornem 0
        if (timeout){
            return 0;
        //si no hi ha mes moviments, o hem arribat al final del arbre, actualitzarem la taula hash i retornarem la heuristica de la fulla
        }else if(moves.isEmpty() || profunditat == 0){
            int heur = heur(ag);
            taula_hash.actualitza(ag, new HashInfo(heur, -1, profunditat, ag.getBoard_color(), ag.getBoard_occupied(), 0, fitxes_totals));
            return heur;
        }
        ...
        if(info != null && info.gettColor() == ag.getBoard_color().toLongArray()[0] && info.gettOcupat() == ag.getBoard_occupied().toLongArray()[0] && profunditat < info.getProfunditat()){
            //en cas de que aquest node ja l'haguem calculat anteriorment
            if (info.getTipusPoda() == 1){
                //si el calcul heuristic hagues sigut de tipus poda alpha, actualitzem el valor de alpha
                alpha = info.getHeuristica();
            }else if (info.getTipusPoda() == 0){
                //si el calcul heuristic hagues sigut per finalitzacio dels fills, retornem directament l'heuristica
        ...
        for (int i : getMovimientos(ag, moves)) {
            ...
            if (game_aux.isGameOver()) {
                if(game_aux.GetWinner() == jugador){
                    //si el joc sacaba i hem guanyat no fa falta calcular mes moviments
                    taula_hash.actualitza(game_aux, new HashInfo(Integer.MAX_VALUE, i, profunditat, ag.getBoard_color(), ag.getBoard_occupied(), 0, fitxes_totals));
                    return Integer.MAX_VALUE;
                }
                
            } else {
                //cridem a la funcio del minimitza i busquem resultats millos dels que tenim guardats
                vella_alpha = nova_alpha;
                nova_alpha = Math.max(nova_alpha, minimitza(game_aux, profunditat - 1, max_profunditat + 1, alpha, beta));
                if(vella_alpha != nova_alpha){//vella_alpha l'utilitzem per actualitzar la millor posicio solament cuan hi hagui un nou valor d'alpha (que sera millor al anterior)
                    millor_posicio = i;
                }
                ...
                if (alpha >= beta) {
                    taula_hash.actualitza(ag, new HashInfo(alpha, -1, profunditat, ag.getBoard_color(), ag.getBoard_occupied(), 1, fitxes_totals));
                    return alpha;
                }
            }
        }
        ...
        taula_hash.actualitza(ag, new HashInfo(alpha, millor_posicio, profunditat, ag.getBoard_color(), ag.getBoard_occupied(), 0, fitxes_totals));
        return nova_alpha;
    }
    ...
    public int minimitza (AbracitosGame ag, int profunditat, int max_profunditat, int alpha, int beta){
        ...
        //si s'ha acabat el temps tornem 0
        if (timeout){
            return 0;
        //si no hi ha mes moviments, o hem arribat al final del arbre, actualitzarem la taula hash i retornarem la heuristica de la fulla
        }else if (moves.isEmpty() || profunditat == 0) {
        ...
        for (int i : getMovimientos(ag, moves)) {
            ...
            if (game_aux.isGameOver()) {
                if(game_aux.GetWinner() == jugador_enemic){
                    ...
                    taula_hash.actualitza(ag, new HashInfo(Integer.MIN_VALUE, i, profunditat, ag.getBoard_color(), ag.getBoard_occupied(), 0, fitxes_totals));
                ...
                if(vella_beta != nova_beta){//vella_beta l'utilitzem per actualitzar la millor posicio solament cuan hi hagui un nou valor de beta (que sera millor al anterior)
                    millor_tirada = i;
                }
                ...
                if (alpha >= beta) {
                    taula_hash.actualitza(ag, new HashInfo(beta, -1, profunditat, ag.getBoard_color(), ag.getBoard_occupied(), 2, fitxes_totals));
                    return beta;
                }
            }
        }
        ...
        taula_hash.actualitza(ag, new HashInfo(beta, millor_tirada, profunditat, ag.getBoard_color(), ag.getBoard_occupied(), 0, fitxes_totals));
        return nova_beta;
    }
    ...
    private class Task implements Callable<AbracitosInfo>{
         ...
         * @throws Exception en cas de fallar el threading
         */
        @Override
